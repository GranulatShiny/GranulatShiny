...
La figure en haut à droite est une courbe d'abondance cumulée en fonction du nombre d'espèces par ordre d'abondance décroissante. L'abondance cumulée se réfère à la somme cumulative des abondances des espèces dans un ensemble de données, en commençant par l'espèce la plus abondante et en ajoutant successivement les abondances des espèces suivantes dans l'ordre décroissant. La courbe d'abondance cumulée permet d'évaluer la diversité et la répartition des espèces dans un écosystème ou dans un échantillon biologique. Plus la courbe est droite, plus la communauté est diversifiée alors qu’une courbe qui monte rapidement puis s'aplatit indique une communauté où quelques espèces sont très abondantes tandis que la plupart des espèces sont rares. Attention la courbe d'abondance cumulée apporte un intérêt lorsqu'il y a de nombreuses espèces différentes. Ici, la courbe est construite à partir du jeu de données d'une concession fictive avec seulement 4 espèces. Elle présente donc un intérêt limité. En pratique, vous ne devriez pas avoir ce genre de résultats avec vos données.
...
#####     [Méthodes paramétrique et non-paramétrique]{.underline}

Le domaine des statistiques existe parce qu'il est impossible de collecter des données auprès de tous les individus concernés par l'objet d'une question donnée (population). La seule solution consiste à collecter des données auprès d'un sous-ensemble des individus concernés (échantillon), mais le véritable objectif est de connaître la "vérité" sur la population au sens statistique. La population est donc approchée en étudiant des variables descriptives de ses caractéristiques. Chaque variable descriptive étudiée est un objet statistique, qui peut être décrit par des indicateurs. Les indicateurs statistiques tels que la moyenne, l'écart-type et les quartiles servent à résumer l'information concernant une variable observée. Lorsque l'on étudie un échantillon considéré comme représentatif, ces indicateurs servent à construire la loi de distribution de la variable étudiée. Chaque indicateur correspond à un paramètre de cette loi de distribution. On considère alors que la loi de distribution obtenue pour cette variable à partir de l'échantillon est applicable à la population. Une statistique estime donc un paramètre. Les méthodes statistiques qui sont dites paramétriques, telles que les modèles linéaires ou les modèles linéaires généralisés, supposent que les données suivent une distribution spécifique (i.e. distribution normale) dans la population sous-jacente. De plus, elles supposent souvent que certains paramètres de cette distribution, comme la moyenne et l'écart-type, sont connus ou peuvent être estimés à partir des données. Contrairement aux méthodes paramétriques, les méthodes statistiques non paramétriques n'imposent pas de suppositions spécifiques sur la forme de la distribution des données.

#####     [Modèles linéaires]{.underline}

Un modèle linéaire classique permet d’étudier la liaison statistique entre une variable réponse ***Y*** et les variables explicatives ***X***. Soit *y<sub>i</sub>* la réponse de l’individu *i* et *x<sub>i</sub>* les valeurs prises par les variables explicatives pour cet individu. La relation entre ***X***, la matrice des variables explicatives, et ***Y***, le vecteur réponse, peut s’écrire sous la forme : $$Y = α + β.X + ε \text{ }\text{ }\text{ avec } ε \sim N(0,\sigma)$$ où ***ε*** représente les résidus du modèle. Ils suivent une loi normale centrée, de variance homogène et sont indépendants des variables explicatives. Le terme ***α*** correspond à l'ordonnée à l'origine et ***β*** est un vecteur composé des coefficients estimés du modèle pour chacune des variables explicatives composant la matrice ***X***. La variable réponse pour un modèle linéaire doit être une variable approximativement normalement distribuée.

#####     [Modèles linéaires généralisés]{.underline}

Les modèles linéaires trouvent une large application mais ne peuvent pas gérer des variables réponses discrètes ou asymétriques, qui violent les hypothèses de ce cadre analytique. Par exemple, les variables réponses de type “comptage” suivent généralement une loi de Poisson, tandis que les variables binaires comme la présence/absence sont associées à la loi binomiale. Les modèles linéaires généralisés (GLM) permettent l'extension du cadre méthodologique de la modélisation linéaire à une classe plus large de types de réponse, comme celles énoncées précédemment. 

Une chose importante à comprendre dans les GLM est la relation entre les valeurs de la variable de réponse ***Y*** (telles que mesurées dans les données et prédites par le modèle dans les valeurs ajustées) et le prédicteur linéaire. Le prédicteur linéaire émerge du modèle linéaire comme une somme de chaque terme du modèle. Le prédicteur linéaire correspond à la variable ***Y*** seulement lors d’un modèle linaire classique suivant une loi normale. Dans le cas de modèle linéaire généralisé, c’est la fonction de lien, *g*, qui relie la valeur ***Y*** à son prédicteur linéaire ***N***. On peut alors écrire le modèle statistique associé tel que $$ N =g(Y)= α + β.X + ε$$ $$\text{et } Y=g^{-1}(N) $$ $$\text{avec } ε \sim N(0,\sigma)$$ La valeur de ***N*** est obtenue en transformant la valeur de ***Y*** par la fonction de lien *g*, et la valeur prédite de ***Y*** est obtenue en appliquant la fonction de lien inverse, *g<sup>-1</sup>* à ***N***. <br> En utilisant différentes lois de distribution et donc différentes fonctions de lien, il est possible d’observer les conséquences sur la distribution des résidus du modèle. La fonction de lien la **plus appropriée** est celle qui produit **les résidus les plus conformes aux hypothèses du modèle linéaire**.

#####     [Modèle linéaire généralisé mixte]{.underline}

Les modèles linéaires mixtes généralisés (GLMM) sont une extension des GLM. Un GLMM est dit "mixte" lorsqu'il comprend au moins un effet "fixe" et au moins un effet "aléatoire". Les effets fixes correspondent aux variables explicatives dont l’effet sur la variable réponse est attendu et pour lesquels l’utilisateur souhaite l’estimation d’un paramètre associé (*β<sub>i</sub>*, la taille de l’effet de la variable explicative i). Les effets aléatoires ne sont pas évalués, ils servent uniquement à indiquer au modèle que les données ne sont pas indépendantes et reflètent une corrélation entre les unités statistiques. D'un point de vue statistique, l’intégration des effets aléatoires permet d'améliorer l'estimation de la déviance résiduelle du modèle en structurant le terme d'erreur (***ε***). Ce faisant, on limite les biais dans l'estimation des paramètres du modèle (***β***) et de leur écart-type. Au final, cela se traduit par des valeurs de paramètres plus proches de la réalité et par des "p-value" ayant une meilleure fiabilité.

#####     [Analyse de variance par permutations]{.underline}

La PERMANOVA, ou Analyse de Variance Multivariée par Permutations, est une méthode statistique qui permet d'analyser les différences entre plusieurs groupes définis par des caractéristiques qualitatives, comme par exemple les différents traitements dans une étude expérimentale.

Contrairement à d'autres méthodes statistiques qui nécessitent de respecter certaines hypothèses sur la distribution des résidus lorsque l'on monte un modèle, la PERMANOVA est une méthode non-paramétrique. Elle se focalise sur une matrice de distance entre les éléments étudiés. Cette méthode permet de travailler avec une ou plusieurs variables réponses et de chercher les effets que peuvent avoir une ou plusieurs variables catégorielles sur les données.

L’objectif de la PERMANOVA est de déterminer s’il existe des différences significatives dans la variabilité entre les groupes, sans quantifier celle-ci. Pour ce faire, elle évalue la variation entre les groupes (variance inter-groupes, notée SS inter dans les sorties de R pour « sum of squares inter-groups », la somme des carrés des écarts à la moyenne de chaque groupe par rapport à la moyenne des groupes) par rapport à la variation à l’intérieur des groupes (variance intra-groupes, notée SS intra dans les sorties de R pour « sum of squares intra-groups », la somme des carrés des écarts à la moyenne de chaque observation par rapport à la moyenne du groupe auquel elle appartient). Une valeur de SS inter élevée suggère des différences importantes entre les moyennes des groupes. A l’inverse, une valeur de SS intra faible indique une similarité accrue des observations au sein de chaque groupe.

La PERMANOVA utilise la permutation aléatoire des catégories ou groupes d'échantillons pour évaluer si les différences observées entre les groupes sont statistiquement significatives. Tout d'abord, une mesure de la différence entre les groupes est calculée à partir des données originales. Ensuite, elle mélange aléatoirement les étiquettes des échantillons et recalcule cette mesure pour voir si les différences persistent même après ce mélange. Ce processus est répété de nombreuses fois pour créer une distribution de cette mesure sous l'hypothèse nulle (absence de différence entre les groupes). En comparant la mesure obtenue à partir des données originales à cette distribution, la PERMANOVA détermine si les différences observées sont probablement réelles ou simplement le fruit du hasard.
...
Dans GranulatShiny, les modèles sont centrés sur la variable traitement car les suivis environnementaux des concessions d’extraction de granulats marins ont pour objectif de caractériser l’impact de l’activité sur les différents compartiments du milieu marin selon l’approche BACI (Before After Control Impact). Par définition, l’approche BACI compare des stations d’échantillonnage témoins (c’est-à-dire non impactées) et des stations d’échantillonnage impactées et teste les différences entre avant et après l’introduction de la perturbation anthropique considérée (ici, l’extraction de granulats marins). Il s’agit d’une approche couramment utilisée dans la surveillance de l’environnement océanique et une approche BACI bien conçue reste l’une des meilleures méthodes pour les programmes de surveillance des effets sur l’environnement. Malheureusement, cette méthode présente plusieurs limites qui compromettent sa capacité à détecter des effets notamment parce que l’océan est spatialement et temporellement dynamique, et que trouver deux emplacements statistiquement identiques l’un à l’autre tout en étant suffisamment éloignés géographiquement pour être statistiquement indépendants constitue un véritable défi. <br>

Selon la méthode de modélisation que vous choisissez, la formulation du modèle diffère. Le GLMM prendra en compte deux variables explicatives fixes, le traitement et la saison, et leur interaction ainsi que deux variables explicatives aléatoires, la campagne et la station. Si l'on prend l'exemple de l'abondance comme variable réponse, la formulation du modèle dans R et de ce fait dans l'interface GranulatShiny est la suivante : $$ Abun \sim traitement * saison + (1\|campagne) + (1\|station)$$ 
Le GLM prendra en compte uniquement les variables explicatives fixes prises en compte dans le GLMM et leur interaction : $$ Abun \sim traitement * saison$$ La PERMANOVA prendra en compte les mêmes variables explicatives que le GLM : $$Abun \sim traitement * saison$$
La variable **traitement** sert à analyser l’impact de l’extraction des granulats sur les populations halieutiques. <br>
La variable **saison**, permet de prendre en compte la saisonnalité des données.<br>
L’effet aléatoire **station** permet de prendre en compte la variabilité spatiale du milieu.<br>
L’effet aléatoire **campagne** permet de prendre en compte la variabilité temporelle du milieu.

Pour un GLMM et un GLM, vous devrez choisir une distribution de probabilité. Par défaut, l’interface propose la dernière distribution de probabilité que vous avez utilisée dans la partie précédente. **Attention** : la méthode utilisée ensuite pour la modélisation est itérative, c'est à dire que le résultat final est obtenu par des ajustements successifs. Il se peut donc que la distribution qui semblait la plus adéquate dans la partie précédente ne soit pas forcément celle qui permettra de mieux faire converger le modèle. Néanmoins l’onglet “diagnostic d’analyse” devrait avoir permis de sélectionner un nombre restreint de distribution possible pour ne pas avoir à toutes les tester ici.<br>

Vous pouvez également conserver ou non l'interaction entre les covariables traitement et saison. **Attention** : si l'interaction n'apporte rien au modèle celle-ci sera retirée automatiquement. Vous pouvez également ajouter d'autres covariables dans votre modèle. Elles seront ajoutées sans interaction avec les autres. Lorsque vous êtes prêt, vous pouvez cliquer sur "démarrer la modélisation".<br>

#####     [Modèle linéaire mixte généralisé]{.underline}

**Paragraphe 8.4.2 du "Protocole halieutique de reference" :**<br>
*"Le recours à l’analyse inférentielle est nécessaire pour évaluer de la variabilité temporelle et spatiale des différents indicateurs des ressources halieutiques avant extraction : utilisation de modèles linéaires généralisés à effets mixtes (GLMMs) avec les variables temporelle et spatiale définies comme effets aléatoires croisés plus un effet saisonnier fixe"*

Le GLMM est la méthode à prioriser. La sortie appelée « summary » dans l’interface reproduit celle du logiciel R pour la ligne de commande correspondante. A partir du menu déroulant, vous pouvez choisir d’afficher le tableau d’analyse de la variance, qui synthétise les résultats de la modélisation en ne fournissant qu’un résumé de l’évaluation de l’importance des effets fixes dans le modèle et de comprendre leur impact sur la variable réponse. Vous pouvez également afficher le résumé exhaustif de ces résultats. Vous pouvez choisir d’afficher les résultats du modèle avant optimisation via le choix initial ou alors le modèle optimisé via le choix final en bas à gauche.<br>
...
Les éléments affichés sont: <br>
[[1]] le type de modèle avec la méthode de calcul utilisée, <br>
[[2]] la loi de probabilité et sa fonction de lien, <br>
[[3]] la ligne de commande complète, qui permet de vérifier que la formulation du modèle correspond bien à l’attente de l’utilisateur.
...
Le BIC est un autre critère de sélection de modèle qui, comme l'AIC, prend en compte à la fois l'ajustement et la complexité du modèle. Cependant, le BIC pénalise plus sévèrement la complexité du modèle que l'AIC. Un modèle avec un BIC plus bas est considéré comme préférable.<br>
**logLik (Log-vraisemblance) :**
La log-vraisemblance est une mesure de l'ajustement du modèle aux données. Elle représente la probabilité que les données observées soient générées par le modèle ajusté. Plus la log-vraisemblance est élevée, meilleure est l'ajustement du modèle aux données. On travaille presque toujours avec le log de la vraisemblance pour des raisons pratiques et parce qu'elle est utilisé pour le calcul de la déviance.<br>
**Deviance :**
La deviance est une mesure de l'ajustement du modèle par rapport à un modèle de référence, souvent un modèle nul. Elle est calculée comme la différence entre la déviance du modèle ajusté et celle du modèle de référence. Une deviance plus faible indique un meilleur ajustement du modèle aux données. On peut la calculer grâce à la vraisemblance (notée L) selon la formule :
...
Les "scaled residuals" sont les résidus standardisés du modèle. Des tests sont effectués dessus afin de vérifier la bonne convergence et le bon ajustement du modèle.

```{r, echo=FALSE, warning=FALSE}
summary(GLMM_summary[["residuals"]])
```

Le tableau des effets aléatoires est spécifique au GLMM. Il renseigne les informations sur cette partie de la formule :<br> (1 \| campagne) + (1 \| station). Comme indiqué ci-dessus, il n’y a pas d’estimation de paramètre associé à ces facteurs mais une évaluation de l’écart-type engendré par ces facteurs sur la valeur de l’ordonnée à l’origine ;
...
**Pr(>|z|):** Cette colonne indique la "p-value" associée à la statistique t pour chaque coefficient. Elle indique la probabilité d'observer les données si l'hypothèse nulle (aucun effet) était vraie. Des valeurs p plus faibles suggèrent une preuve plus forte contre l'hypothèse nulle et indiquent que le coefficient est statistiquement significatif.<br>

```{r, echo=FALSE, warning=FALSE}
GLMM_summary[["coefficients"]]
```

La colonne "Estimate" renseigne sur la valeur moyenne que prend la variable étudiée (dans notre exemple l'abondance totale) par modalité des variables explicatives. Dans notre exemple, l'abondance est expliquée par la variable "traitement" avec 2 modalités (impact, Sans impact) et la variable "saison" avec 4 modalités (winter, spring, summer, autumn). La ligne (Intercept) correspond à une valeur de base (soit l’ordonnée à l’origine). Cette valeur de base est associée à une modalité de chacune de nos variables. La colonne “Pr(>|z|)” permet d’évaluer si le changement est significatif en vérifiant si la valeur est inférieure à 0.05. Dans cet exemple, c’est l’interaction entre la saison et le traitement qui apporte des changements significativement différents : Si ces deux variables étaient considérées séparément, leur effet sur l’abondance ne serait pas visible. <br>
Ainsi, ici :<br>
 - **En hiver et dans une zone impactée** le logarithme de l'abondance (car on utilise une loi Lognormale) vaut en moyenne 10.68 ;<br>
 - pour connaitre la valeur du logarithme de l'abondance **en hiver et sans impact**, il faut additionner la valeur Intercept et la valeur de l'estimate de la ligne "traitementSans impact" soit 10.68 + 0.12 qui vaut 10.8 ;
 - pour obtenir la valeur du logarithme de l'abondance **en été et avec impact**, il faut additionner la valeur Intercept à la valeur de l'estimate de la ligne "saisonSummer" soit 10.68 + (-0.16) soit 10.52 ; 
 - Enfin, en additionnant la valeur Intercept avec celle des lignes "traitementSans impact", "saisonSummer", "traitementSans impact:saisonSummer", on obtient la valeur du logarithme de l'abondance **en été et sans impact** soit 10.68 + 0.12 + (-0.16) + 1.06 qui donne 11.70. <br> 

La dernière section montre les corrélations entre les termes à effet fixe du modèle. Chaque ligne et chaque colonne représentent un terme à effet fixe, et les valeurs du tableau sont les corrélations entre ces termes. Ces corrélations sont calculées sur la base de la matrice de covariance des estimations des effets fixes. Elles indiquent comment les effets estimés des différents facteurs fixes du modèle sont liés les uns aux autres.

Analyse a posteriori du modèle GLMM sur l'abondance.

Après avoir obtenu les résultats de la modélisation, l'application propose une série de graphiques pour diagnostiquer la qualité de l'ajustement du modèle, en se concentrant sur l'analyse des résidus. Ces graphiques sont générés à l'aide du package DHARMa ("Residual Diagnostics for Hierarchical (Multi-level/Mixed) Regression Models") dans R.

```{r, echo=FALSE, warning=FALSE}
plot(simulateResiduals(GLMM))
```

Le graphique de gauche, appelé "QQ plot residual" soit graphique quantile-quantile des résidus, est une représentation des résidus attendus par rapport aux observations réelles. Dans ce graphique, chaque point représente un résidu calculé par le modèle pour une observation donnée. Idéalement, ces points devraient suivre de près une ligne rouge diagonale, ce qui signifierait que les résidus sont distribués de manière normale. Si les points s'éloignent de manière significative de cette ligne rouge, cela suggère une mauvaise adéquation du modèle aux données observées.
...
<br> Lorsque la modélisation par GLMM ne converge pas, il faut lui préférer des méthodes de modélisation associées à  des algorithmes de calcul plus simples, c'est-à-dire avec moins de paramètres à estimer. L'outil GranulatShiny en propose deux : le GLM et la PERMANOVA.<br> **NOTA BENE** : lorsque le jeu de données utilisé pour la modélisation est constitué de 30 observations non nulles ou moins, il est préférable de s'en tenir à la méthode la moins coûteuse en termes de calcul, à savoir la PERMANOVA.
...
Le GLM sort des résultats proches de celui du GLMM et s'analyse de la même manière. Cependant, il ne prend pas en compte les effets aléatoires induits par l'environnement ou la méthode utilisée et sera donc moins précis.

#####     [Analyse de variance par permutations]{.underline}

Méthode de PERMANOVA intégrée dans l'application

Dans le cas de Granulatshiny, la PERMANOVA est appliquée sur une matrice colonne regroupant la plupart du temps des abondances ou des biomasses. Dans le cas général, lorsque la variable réponse est issue de relevés floristiques ou faunistiques, le zéro correspond à une absence de l'espèce considérée. Alors que la coprésence d'une espèce peut indiquer des conditions similaires en termes de niches écologiques et que la présence/absence peut opposer deux niches, la double absence (double 0) peut être due à différentes raisons (espèces rares non collectées, échantillonnage hors niche écologique pour les deux espèces, etc.).  Dans ce dernier cas, ce double 0 est donc classiquement non pris en compte pour estimer la ressemblance entre deux éléments notamment dans un esprit de caractérisation de communauté biologique. Ainsi, dans notre cas, les données sur lesquels sera appliquée la PERMANOVA sont quantitatives et les doubles zéros ne sont pas considérés. La méthode de construction de la matrice de distance, objet de l’analyse, qui semble la plus approprié est donc celle du coefficient de Bray-Curtis.<br>
L’indice de dissimilarité de Bray-Curtis, est utilisé en écologie et biologie pour évaluer la dissimilarité entre deux échantillons donnés, en termes d’abondance de taxons présents dans chacun de ces échantillons. Il est compris entre 0 (les deux échantillons ont la même composition) et 1 (les échantillons sont totalement dissemblables). La dissimilarité de Bray-Curtis est souvent utilisée dans la littérature. Sa distribution est asymétrique et semi-métrique. Elle se calcule comme ceci : 
 $$ d_{jk}=\frac{\sum_{i} |x_{ij}-x_{ik}|}{\sum_{i} (x_{ij}+x_{ik})} $$ où *i = colonne; j,k = lignes comparées; x = valeurs d'abondances*

Dans le cas où la matrice d’entrée n’a qu’une seule colonne, soit une seule espèce, et qu'il exsite des zéros dans les valeurs d’entrées, il arrive parfois que le dénominateur soit égal à zéro ce qui crée une erreur dans la matrice de distance. C’est le cas pour tous les indicateurs habituellement utilisés pour des données d'abondance qui pondèrent leur distance en fonction de l’abondance totale dans les sites comparés comme le fait la dissimilarité de Bray-Curtis. Cette méthode ne pouvant s'utiliser dans notre cas, un autre coefficient de calcul de distance a été recherché. Celui-ci ne devait également pas tenir compte des doubles zéros sur des quantitatives.

La méthode retenue est la métrique du khi-carré (χ²). Son utilisation est recommandée lorsque les espèces rares sont de bons indicateurs de conditions écologiques particulières. Pour appliquer cette méthode, il faut d'abord standardiser les données selon la méthode du khi-carré comme ceci : $$ x'_{ij}=\frac{x_{ij}}{\sum x_{j} * \sqrt \sum x_{i}} $$ où *i = colonne; j = ligne; x = valeurs d'abondances*
...
L’inconvénient de cette méthode, le calcul des effets se produisant sur des données transformées, est qu’il n’est pas possible de quantifier directement l’impact d’un effet sur la variable initiale. On ne peut donc pas dire si l’effet d’une variable explicative est plus ou moins important sur la donnée initiale car celui-ci s’applique à la donnée transformée. Par contre, si un effet est considéré significatif sur les données transformées alors il l’est également sur les données initiales.

Utilisation de la PERMANOVA

Lorsque l'on cherche à évaluer l'impact de l'extraction sur une espèce peu échantillonnée, il n'est pas possible d'utiliser un GLMM ou un GLM. Ces modèles sont sensibles à une trop forte présence de zéros dans les données et il n'est pas possible pour ceux-ci de converger. Dans l'application, lorsque ce cas de figure se présente, il est conseillé d'utiliser la PERMANOVA. Néanmoins, comme cela a été expliqué ci-dessus, pour une variable d'analyse avec de nombreux zéros, la PERMANOVA doit être utiliser avec précaution et la métrique de calcul de distance choisi avec précision. Ces données compliquées à manipuler sont appelés des données "zero inflated" et des modèles spécifiques existent pour leur analyse. Le modèle le plus couramment utilisé est le modèle "delta". Celui-ci va analyser en deux temps les données en commençant par une analyse en présence/absence puis sur les valeurs quantitatives associées à la présence. Ce type de modèle n'est pas implémenté actuellement dans GranulatShiny.
...
Dans le tableau de sortie de la PERMANOVA, on retrouve le nombre de permutations et la formule du modèle. Ensuite on retrouve plusieurs indicateurs associés à chaque covariable explicative.<br> **Df (degrés de liberté)**: Cette colonne indique les degrés de liberté associés à chaque terme du modèle.<br> **SumOfSqs (Somme des carrés)**: Cette colonne indique la somme des distances au carré entre les observations dans l'espace multivarié.<br> **R2 (R-carré)**: Cette colonne indique la proportion de variance expliquée par chaque terme du modèle. Par exemple, pour "traitement", 8,34 % de la variation des données peut être expliquée par le facteur traitement.<br> **F (statistique F)** : Cette colonne indique la statistique F pour chaque terme, qui vérifie si la variation expliquée par ce terme est significativement plus importante que ce que l'on attendrait du hasard. Des valeurs F plus élevées indiquent des preuves plus solides pour rejeter l'hypothèse nulle d'absence d'effet.<br> **Pr(\>F) (p-value)**: Cette colonne indique la "p-value" associée à la statistique F pour chaque terme. Elle indique la probabilité d'observer les données si l'hypothèse nulle d'absence d'effet (c'est-à-dire si toutes les moyennes des groupes sont égales) était vraie. Des "p-value" plus faibles suggèrent une preuve plus forte contre l'hypothèse nulle et indiquent que le terme est un prédicteur significatif de la variation.<br> 
...
Si vous avez plusieurs covariables vous devez les fixer afin de pouvoir visualiser le graphique. <br> Dans l'exemple d'un GLM qui regarde l'abondance totale en fonction du traitement et de la saison, voici le graphique obtenu :
...
Cette partie est en cours de développement. Une première version de cet élément a été proposé dans une version antérieure. L’outil n’étant pas généralisable, celui-ci a été retiré pour assurer la stabilité actuelle de l’application. <br>
