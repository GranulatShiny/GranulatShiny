...
Dans le cas de Granulatshiny, la PERMANOVA est appliquée sur une matrice colonne regroupant la plupart du temps des abondances ou des biomasses. Lorsqu'il s'agit d'une variable issue de relevés floristiques ou faunistiques, le zéro induit  une absence de l'espèce considérée. Alors que la coprésence d'une espèce peut indiquer des conditions similaires en termes de niches écologiques et que la présence/absence peut opposer deux niches, la double absence (double 0) peut être due à différentes raisons (espèces rares non collectées, échantillonnage hors niche écologique pour les deux espèces, etc.).  Dans ce dernier cas, ce double 0 est donc classiquement non pris en compte pour estimer la ressemblance entre deux éléments notamment dans un esprit de caractérisation de communauté biologique. Ainsi les données sur lesquels sera appliquée la PERMANOVA sont quantitatives et les doubles zéros ne sont pas considéré donc la méthode qui semble la plus approprié est celle du coefficient de Bray-Curtis.<br>
...
Lorsque l'on cherche a évalué l'impact de l'extraction sur une espèce peu pêché, il n'est pas possible d'utiliser un GLMM ou une GLM. Ces modèles sont sensibles à une trop forte présence de zéros dans les données et il n'est pas possible pour ceux-ci de converger. Dans l'application, lorsque ce cas de figure ce présente, il est conseillé d'utiliser la PERMANOVA. Néanmoins, comme cela a été montré ci-dessus, pour une varaible d'analyse avec de nombreux zéros, la PERMANOVA doit être utiliser avec précaution et la métrique de calcul de distance choisi avec précision. Ces données compliquées à manipuler sont appelés des données "zero inflated" et des modèles spécifiques existent pour leur analyse. Le modèle le plus couramment utilisé est le modèle "delta". Celui-ci va analyser en deux temps les données en commençant par une analyse en absence/présence puis sur les valeurs quantitatives associées à la présence. Ce type de modèle n'est implanté actuellement dans GranulatShiny.
